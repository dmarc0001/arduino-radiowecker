...
namespace logger
{
  ...
  void Elog::outputSd( LogLineEntry &logLineEntry, char *logLineMessage )
  {
    ...
    if ( sdConfigured )
    {
      if ( sdCardPresent )
      {
        ...
        if ( svc->sdFileHandle )
        {                       // Are we working on a valid file?
          ...
          {  // If not everything is written, then the SD must be ejected.
            sdCardPresent = false;
        ...
        {  // If we dont have a valid filehandle we try do create it periodically
      ...
      {  // If no sd card is present, try to connect to it.
        loggerStatus.sdMsgNotWritten++;
        reconnectSd();
      }
    }
  ...
  void Elog::configureSd( SPIClass &_spi,
                          uint8_t _cs,
                          uint32_t _speed,
  ...
  */
  ...
  {
    if ( !svc->sdFileHandle )
    {  // Only do something if we dont have a valid filehandle
...
#ifndef NOT_SYSLOG
    service.syslogEnabled = false;  // defauld disabled
#endif
...
#ifndef NOT_SYSLOG
  /**
   * define udp settingds for syslog
   * instance an WiFiUDP object in your program
   */
  void Elog::setUdpClient( UDP &client,
                           const char *server,
                           uint16_t port,
                           ...
                           uint8_t protocol )
  {
    ...
    service._port = port;
    service._deviceHostname = ( deviceHostname == NULL ) ? SYSLOG_NILVALUE : deviceHostname;
    service._appName = ( appName == NULL ) ? SYSLOG_NILVALUE : appName;
    service._pri = priDefault;
    service._isOnline = false;
  }
  ...
  void Elog::addSyslogLogging( const Loglevel wantedLogLevel )
  {
    writerTaskStart();  // Make sure that the writerTask is running
    if ( !service.syslogEnabled )
    {
      logInternal( INFO, "Adding syslog logging." );
      service.syslogWantedLoglevel = wantedLogLevel;
      service.syslogEnabled = true;
      // service._isOnline = true;
    }
    else
    {
      logInternal( ERROR, "You can only add syslog logging once" );
    }
  }

  void Elog::setSyslogOnline( bool _isOnline )
  {
    if ( service.syslogEnabled )
      service._isOnline = _isOnline;
    else
      service._isOnline = false;
  }
  ...
  void Elog::reportStatus()
  {
    ...
    if ( logRingBuff.percentageFull() < 50 )
    {  // When buffer under half full, we clear "full warning".
    ...
    {
      logInternal( DEBUG, "Status (Buffer): msgs added %d, discarded %d, Max Buffer usage %d%%", loggerStatus.bufferMsgAdded,
                   loggerStatus.bufferMsgNotAdded, maxBuffPct );

      if ( serialEnabled )
      {
        logInternal( DEBUG, "Status (Serial): msgs written %d (%d bytes)", loggerStatus.serialMsgWritten,
      ...
      if ( sdConfigured )
      {
        ...
        logInternal( DEBUG, "Status (SD): msgs written %d (%d bytes), discarded %d, free space %d bytes, card present: %s",
                     loggerStatus.sdMsgWritten, loggerStatus.sdBytesWritten, loggerStatus.sdMsgNotWritten,
                     sdCardPresent ? freeSpace : 0, sdCardPresent ? "yes" : "no" );
      }
#endif
#ifndef LOGGER_DISABLE_SPIFFS
      if ( spiffsMounted )
      {
        logInternal( DEBUG, "Status (SPIFFS): msgs written %d (%d bytes), discarded %d, free space %d bytes",
                     loggerStatus.spiffsMsgWritten, loggerStatus.spiffsBytesWritten, loggerStatus.spiffsMsgNotWritten,
  ...
  /* All public settings for the Logger library is set here. Remember to call this before calling any
     ...
     Parameters are optional. Parameters:
      ...
      discardMsgWhenBufferFull: If true messages will be discarded if buffer full. Otherwise the logging will depleat the buffer before
     returning. sdReconnectEvery: If connection to sd card is lost, it wil reconnect every x ms sdSyncFilesEvery: cache is written to
     sd card evey x ms sdTryCreateFileEvery: if a file could not be created it will retry every x ms reportStatusEvery: if debugging in
     this lib is enabled, the buffer status will be logged every x ms
  ...
  void Elog::writerTaskStart()
  {
    ...
    if ( started == false )
    {
      logInternal( INFO, "Allocating buffer for %d log messages. Max message size %d chars. %d bytes of memory used",
                   settings.maxLogMessages, settings.maxLogMessageSize, settings.maxLogMessages * settings.maxLogMessageSize );
      logRingBuff.createBuffer( settings.maxLogMessages, settings.maxLogMessageSize );

      logInternal( DEBUG, "Creating Writer task" );
      xTaskCreate( writerTask,   // Task function.
  ...
  void Elog::writerTask( void *parameter )
  {
    ...
    while ( true )
    {
      while ( writerTaskHold )
      {  // Do nothing if we are instructed to pause.
        vTaskDelay( 1 );
      }
...
#endif
#ifndef NOT_SYSLOG
        if ( logLineEntry.logSyslog )
        {
          sendLog( logLineEntry, logLineMessage );
        }
#endif
      }
...
#ifndef NOT_SYSLOG
  void Elog::sendLog( LogLineEntry &logLineEntry, char *logLineMessage )
  {
    int result;

    if ( ( logLineEntry.service->_server == NULL && logLineEntry.service->_ip == INADDR_NONE ) || logLineEntry.service->_port == 0 ||
         !logLineEntry.service->_isOnline )
      return;

    if ( logLineEntry.service->_server != NULL )
    {
      // logInternal( ERROR, "log to syslog via %s:%d...", logLineEntry.service->_server, logLineEntry.service->_port );
      result = logLineEntry.service->_client->beginPacket( logLineEntry.service->_server, logLineEntry.service->_port );
    }
    else
    {
      // logInternal( ERROR, "log to syslog via %s:%d...", logLineEntry.service->_ip.toString(), logLineEntry.service->_port );
      result = logLineEntry.service->_client->beginPacket( logLineEntry.service->_ip, logLineEntry.service->_port );
    }
    ...
    logLineEntry.service->_client->print( '<' );
    logLineEntry.service->_client->print( logLineEntry.service->_pri );
    if ( logLineEntry.service->_protocol == SYSLOG_PROTO_IETF )
    {
      logLineEntry.service->_client->print( F( ">1 - " ) );
    }
    else
    {
      logLineEntry.service->_client->print( F( ">" ) );
    }
    logLineEntry.service->_client->print( logLineEntry.service->_deviceHostname );
    logLineEntry.service->_client->print( ' ' );
    logLineEntry.service->_client->print( logLineEntry.service->_appName );
    if ( logLineEntry.service->_protocol == SYSLOG_PROTO_IETF )
    {
      logLineEntry.service->_client->print( F( " - - - \xEF\xBB\xBF" ) );
    }
    else
    {
      logLineEntry.service->_client->print( F( "[0]: " ) );
    }
    logLineEntry.service->_client->print( logLineMessage );
    logLineEntry.service->_client->endPacket();
  }
  ...
  void Elog::log( const Loglevel logLevel, const char *format, ... )
  {
    ...
    if ( service.sdEnabled || service.serialEnabled || service.spiffsEnabled || service.syslogEnabled )
    {
      ...
      Loglevel wantedLogLevelSyslog = service.syslogWantedLoglevel;
      ...
      logLineEntry.logSyslog = ( logLevel <= wantedLogLevelSyslog && service.syslogEnabled && service._isOnline );

      // For optimization va-args are checked and handled in here. It's cpu intensive. Only if needed!
      if ( logLineEntry.logFile || logLineEntry.logSerial || logLineEntry.logSpiffs || logLineEntry.logSyslog )
      {
        logLineEntry.loglevel = logLevel;
  ...
  void Elog::addToRingbuffer( const LogLineEntry &logLineEntry, const char *logLineMessage )
  {
    ...
    {
      if ( settings.discardMsgWhenBufferFull )
      {  // BUFFER FULL - bad, it will just be discarded.
        loggerStatus.bufferMsgNotAdded++;
      }
      else
      {
        while ( logRingBuff.isFull() )
        {  // Get one space in buffer to add the message
          yield();
        }
  ...
  /*  This should be called to set up logging to spiffs flash memory. Parameters:
      spiffsSyncEvery: How often the dirty cache is written to filesystem. (def 5sek). Longer is better performance, but you can loose
     data spiffsCheckSpaceEvery: How often free disk space is checked to do cleanups. This takes performance - not too often (def
     20sek) spiffsMinimumSpace: When disk space is checked, we want to remove old logs until we have this free (def 50k)
  */
  ...
  void Elog::spiffsEnsureFreeSpace( bool checkImmediately )
  {
    ...
    if ( checkImmediately || ( millis() - lastStatus > settings.spiffsCheckSpaceEvery ) )
    {
      ...
      while ( ( LittleFS.totalBytes() - LittleFS.usedBytes() ) < settings.spiffsMinimumSpace )
      {
        ...
        while ( strlen( file.name() ) != 5 )
        {  // Search for logfile (always 5 chars long names)
  ...
  void Elog::spiffsListLogFiles( Stream &serialPort )
  {
    ...
    while ( file )
    {
      ...
      {  // All log files are 5 chars long 0000x
  ...
  uint8_t Elog::getTimeStringReal( uint32_t milliseconds, char *output )
  {
    ...
    uint16_t length = sprintf( output, "%04d-%02d-%02d %02d:%02d:%02d %03d", pt.year, pt.month, pt.day, pt.hour, pt.minute, pt.second,
                               pt.millisecond );
    return length;
  }
