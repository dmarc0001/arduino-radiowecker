...
namespace logger
{
  ...
  {
    if ( running )
    {         // We can only stop a running timer
  ...
  void Etimer::lap()
  {
    ...
    else
    {
      ...
      {  // All laps full, print and restart the timer.
  ...
  void Etimer::show()
  {
    ...
    if ( running )
    {  // If timer is running, we stop it before showing it.
      end();
    }

    for ( uint8_t lap = 0; lap < currentLap; lap++ )
    {  // Traverse all the recorded laps
      if ( lap == 0 )
      {
