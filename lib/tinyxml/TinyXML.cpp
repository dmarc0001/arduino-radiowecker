  ...
  while ( !bMatch )
  {
    ...
    switch ( chToParse )
    {
      ...
      case alpha:
        ...
        break;
      case alphanum:
        if ( isAlpha( ch ) || isNumeric( ch ) || ( ch == ':' ) || ( ch == '_' ) || ( ch == '-' ) )
          bMatch = true;
        break;
      case quote:
        if ( ch == '"' || ch == '\'' )
...
void TinyXML::action( uint8_t ch, uint8_t actionType )
{
  ...
  switch ( actionType )
  {
    ...
    case cleardata:
      ...
      break;
    case storeifneeded:
      if ( dataBufferPtr < maxDataLen )
        dataBuffer[ dataBufferPtr++ ] = ch;
      break;
    case starttagname:
      dataBuffer[ dataBufferPtr ] = 0;  // terminate the text
      // call back if the previous tag text is required
      tagBuffer[ tagBufferPtr ] = 0;
      if ( tagBufferPtr && dataBufferPtr )
        Xcb( STATUS_TAG_TEXT, ( char * ) tagBuffer, tagBufferPtr, ( char * ) dataBuffer, dataBufferPtr );
...
#endif
      if ( tagBufferPtr < TAGBUFFERMAX )
        tagBuffer[ tagBufferPtr++ ] = '/';
      else
        action( ch, error );
      break;
    case cleartagname:
    ...
    case removelasttag:
      ...
      while ( tagBufferPtr && tagBuffer[ --tagBufferPtr ] != '/' )
        ;  // as we error if tagBuffer overflows then this will be safe
#if DEBUG > 3
      tagBuffer[ tagBufferPtr ] = 0;
    ...
    case addtochktagname:
      if ( checkTagBufferPtr < CHECKTAGMAX )
        checkTagBuffer[ checkTagBufferPtr++ ] = ch;
      else
        action( ch, error );
      break;
